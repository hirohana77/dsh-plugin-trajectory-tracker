window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			".dsh-tt-rows{display:flex;flex-direction:column;gap:4px;max-height:200px;overflow-y:auto;min-width:240px}",
			".dsh-tt-row{display:flex;justify-content:space-between;gap:16px;font-size:12px;line-height:18px;color:var(--dsw-alias-label-secondary)}",
		...
		function toolCallInfo(n, now) {
			...
			return { name, running, durationMs, startMs, step: nodeStepOf(n) };
		}
