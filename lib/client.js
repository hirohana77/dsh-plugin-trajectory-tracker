window.__ModuleLoader__.load({
	...
	factory: (require) => {
		...
		const CSS = [
			...
			"@keyframes dsh-tt-quick-pop{from{opacity:0;transform:translateY(2px)}to{opacity:1;transform:translateY(0)}}",
			".dsh-tt-row[data-hint]:hover::after{content:attr(data-hint);position:absolute;left:8px;bottom:calc(100% + 2px);background:#ffffff !important;color:#0f172a !important;border:1px solid #cbd5e1 !important;padding:2px 6px;border-radius:4px;font-size:11px;font-weight:400;line-height:16px;white-space:nowrap;max-width:340px;overflow:hidden;text-overflow:ellipsis;box-shadow:0 2px 8px rgba(0,0,0,.15);pointer-events:none;z-index:1200;animation:dsh-tt-quick-pop .12s ease-out .1s forwards;opacity:0}",
